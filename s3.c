...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(filename, "__TAR__") == 0) {
            printf("[%s] TAR request received — creating txt.tar...\n", SUB_DIR);
            char cmd[512];
            snprintf(cmd, sizeof(cmd), "cd %s/%s && find . -name \"*.txt\" | tar -cf txt.tar -T -", BASE_DIR, SUB_DIR);
            system(cmd);

            char path[512];
            snprintf(path, sizeof(path), "%s/%s/txt.tar", BASE_DIR, SUB_DIR);
