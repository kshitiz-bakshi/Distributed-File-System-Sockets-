...
//S2
#define PORT 5001            // s2: 5001, s3: 5002, s4: 5003
#define SUB_DIR "s2"         // s2: "s2", s3: "s3", s4: "s4"
...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(filename, "__TAR__") == 0) {
            printf("[%s] TAR request received — creating pdf.tar...\n", SUB_DIR);
            char cmd[512];
            snprintf(cmd, sizeof(cmd), "cd %s/%s && find . -name \"*.pdf\" | tar -cf pdf.tar -T -", BASE_DIR, SUB_DIR);
            printf("[%s] Running: %s\n", SUB_DIR, cmd);
            system(cmd);
        
            char path[512];
            snprintf(path, sizeof(path), "%s/%s/pdf.tar", BASE_DIR, SUB_DIR);
            FILE *fp = fopen(path, "rb");
            if (!fp) {
                perror("[SERVER] Tar file open failed");
                long zero = 0;
            ...
            fclose(fp);
            remove(path);
            printf("[%s] Sent pdf.tar (%ld bytes)\n", SUB_DIR, size);
            close(newsock);
            continue;
        }        
